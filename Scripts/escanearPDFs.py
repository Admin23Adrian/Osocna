...
def carga_factura(factura, pdf, tipo_comprob, punto_venta, letra, ruta_pdf):
    ...
    # print(url_pdf)
    solicitud = requests.post(url = url_pdf, headers = headers_pdf, files = factura_pdf, verify = False)
        ...
        if os.path.isdir(ruta_cda_carpeta) == True:
            ...
            if adjuntos_modificados != []:
                for presupuesto_mod in adjuntos_modificados:
                    ...
                    request = requests.post(url = url_adjuntos, headers = headers_adjuntos, files = adjunto_a_subir, verify = False)
                    print(f"Status de Subir Adjuntos: {request.status_code}, factura: {factura}")
...
def recorrer_carpetas(factura, tipoCompr, punto_vent, letra):
    ...
    carpeta_factura = f"00{pv}A{factura}"
